...
const LoadingAnimation = ({ onComplete }: LoadingAnimationProps) => {
  ...
  useEffect(() => {
    ...
    // First render - wait for header to be available - SLOWED DOWN by doubling the time
    setTimeout(() => {
      ...
      if (headerPosition) {
        ...
        // Start the fade-out transition after logo has moved - SLOWED DOWN
        setTimeout(() => {
          setFadeOut(true);
        }, 1000);
      } else {
    ...
    // Trigger the onComplete callback once animation is done - SLOWED DOWN
